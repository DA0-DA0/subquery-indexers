...
echo "[backup cron] Backing up database..."
...
if [ -n "$BACKUP_PASSPHRASE" ]; then
  echo "[backup cron] Encrypting backup..."
...
echo "[backup cron] Uploading backup to $BACKUP_PATH..."
aws $aws_args s3 cp "$local_file" "$s3_uri"
rm "$local_file"

deleteAfter="${BACKUP_RETAIN:-"10 days"}"
echo "[backup cron] Deleting backups older than $deleteAfter..."
...
echo "[backup cron] Backup complete."
