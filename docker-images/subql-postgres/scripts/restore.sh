...
pidof -o %PPID -x $0 >/dev/null && echo "[restore] ERROR: Script $0 already running" && exit 1
...
# We have permission to save dump in home folder.
cd $HOME
...
if [ -z "$key_suffix" ]; then
  ...
  echo "[restore] Fetching backup from S3..."
